...
from api.schemas.user import User, UserOut, UserCreate
from api.schemas.patient import Patient, PatientRegister, PatientPrivate, PatientPublic
from api.schemas.specialist import Specialist, SpecialistRegister, SpecialistOut
from api.db.models.user import individual_serial, list_serial
from api.db.client import users_collection
...
async def register(patient: PatientRegister = Depends(), avatar: Optional[UploadFile] = File(None)):
    ...
    if avatar:
        ...
        file_path = os.path.join("api", "uploads", "avatars", file_name)
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        with open(file_path, "wb") as buffer:
            shutil.copyfileobj(avatar.file, buffer)
        patient_dict["avatar_url"] = f"/uploads/avatars/{file_name}"
    else:
...
async def register_specialist(specialist: SpecialistRegister = Depends(), avatar: Optional[UploadFile] = File(None)):
    ...
    if avatar:
        ...
        with open(file_path, "wb") as buffer:
            shutil.copyfileobj(avatar.file, buffer)
        specialist_dict["avatar_url"] = f"/uploads/avatars/{file_name}"
    else:
